...
export class AdminRolesBusinessLogic {
  validateAllRolesExist(requestedIds: string[], foundRoles: Role[]): void {
    ...
    const missingRoleIds = requestedIds.filter(
      (id) => !foundRoleIds.includes(id),
    );

    if (missingRoleIds.length > 0) {
      throw new NotFoundException(
        `Belirtilen ID'lere sahip roller bulunamadı: ${missingRoleIds.join(', ')}`,
      );
    }
  ...
  validateRoleExists(
    role: GetByIdRolesResponseDto | undefined,
    id: string,
  ): void {
    if (!role) {
  ...
  validatePermissionsExist(
    permissions: FindByIdsPermissionsResponseDto[],
  ): void {
  ...
  validateNotSoftDeleted(role: GetByIdRolesResponseDto): void {
    if (role.deletedAt) {
      throw new BadRequestException(
        `Rol '${role.name}' zaten soft delete yapılmış.`,
      );
    }
  }
  ...
  validateSoftDeleted(role: GetByIdRolesResponseDto): void {
    if (!role.deletedAt) {
      throw new BadRequestException(
        `Rol '${role.name}' soft delete yapılmadığı için geri yüklenemez.`,
      );
    }
  }
}
