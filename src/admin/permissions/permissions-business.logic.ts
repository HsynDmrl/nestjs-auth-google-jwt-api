...
export class PermissionsBusinessLogic {
  validateAllPermissionsExist(
    requestedIds: string[],
    foundPermissions: Permission[],
  ): void {
    const foundPermissionIds = foundPermissions.map(
      (permission) => permission.id,
    );
    const missingPermissionIds = requestedIds.filter(
      (id) => !foundPermissionIds.includes(id),
    );

    if (missingPermissionIds.length > 0) {
      throw new NotFoundException(
        `Belirtilen ID'lere sahip yetkiler bulunamadı: ${missingPermissionIds.join(', ')}`,
      );
    }
    ...
    if (duplicateIds.length > 0) {
      throw new BadRequestException(
        `Tekrar eden ID'ler bulundu: ${duplicateIds.join(', ')}`,
      );
    }
  }
  ...
  validatePermissionsExist(permissions: Permission[], ids?: string[]): void {
    if (permissions.length === 0) {
      if (ids) {
        throw new NotFoundException(
          `Belirtilen ID'lere sahip yetkiler bulunamadı: ${ids.join(', ')}`,
        );
      }
  ...
  validatePermissionNameUniqueness(
    existingPermission: Permission | undefined,
  ): void {
  ...
  validateNotSoftDeleted(permission: GetByIdPermissionsResponseDto): void {
    if (permission.deletedAt) {
      throw new BadRequestException(
        `Yetki '${permission.name}' zaten soft delete yapılmış.`,
      );
    }
  }
  ...
  validateSoftDeleted(permission: GetByIdPermissionsResponseDto): void {
    if (!permission.deletedAt) {
      throw new BadRequestException(
        `Yetki '${permission.name}' soft delete yapılmadığı için geri yüklenemez.`,
      );
    }
  }
