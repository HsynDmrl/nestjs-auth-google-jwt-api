...
export class AdminUsersBusinessLogic {
  validateUserExists(
    user: GetByIdAdminUsersResponseDto | undefined,
    id: string,
  ): void {
    if (!user) {
      throw new NotFoundException(
        `Kullanıcı ID'si ${id} olan kullanıcı bulunamadı.`,
      );
    }
  }
  ...
  validateUserNamesUniqueness(
    existingUser: GetByIdAdminUsersResponseDto | undefined,
  ): void {
    if (existingUser) {
  ...
  validateNotSoftDeleted(user: GetByIdAdminUsersResponseDto): void {
    if (user.deletedAt) {
      throw new NotFoundException(
        `Kullanıcı '${user.name}' zaten soft delete yapılmış.`,
      );
    }
  }
  ...
  validateSoftDeleted(user: GetByIdAdminUsersResponseDto): void {
    if (!user.deletedAt) {
      throw new NotFoundException(
        `Kullanıcı '${user.name}' zaten soft delete yapılmamış.`,
      );
    }
  }
}
