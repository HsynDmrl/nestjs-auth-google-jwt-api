...
@Injectable()
export class GooglePlayProvider implements BillingProvider {
  readonly platform = SubscriptionPlatform.GOOGLE_PLAY;
  ...
  private buildResult(
    ...
    if (!planCode) {
      throw new BadRequestException('Google Play ürün kodu tanınmadı.');
    }
    ...
    const environment =
      payload.environment ?? SubscriptionEnvironment.PRODUCTION;
  ...
  private assertDemoMode(): void {
    if (!this.demoMode) {
      throw new BadRequestException(
        'Google Play doğrulaması yalnızca demo modunda kullanılabilir.',
      );
    }
  }
}
