...
export class SeedService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const userRoleExists = await this.roleRepository.findOne({
      where: { name: 'user' },
      relations: ['permissions'],
    });
    if (!userRoleExists) {
      const userRolePermissions = await this.permissionRepository.find({
        where: userPermissions.map((name) => ({ name })),
      });
